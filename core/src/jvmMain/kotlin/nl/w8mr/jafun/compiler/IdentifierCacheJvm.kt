...
class JvmClassInfo(val jClass: Class<*>) : ClassInfo
...
actual object IdentifierCache : SymbolMap {
        ...
        return jMethod?.let {
            ...
            val precedence =
                jMethod.annotations.filterIsInstance<FunctionPrecedence>().map(FunctionPrecedence::precedence)
                    .firstOrNull() ?: 10
            val jfClass = findOrAddClassInternal(jClass)
            ...
            val method =
                Type.JFMethod(
                    ...
                    name,
                    rtn,
                    java.lang.reflect.Modifier.isStatic(jMethod.modifiers),
    ...
    private fun findOrAddClassInternal(jClass: Class<*>): Type.JFClass {
    ...
    actual fun findClass(path: String): Type.JFClass {
        ...
        return Type.JFClass(name, currentPackage)
    }

    actual override fun findOrAddClass(clazz: ClassInfo): Type.JFClass {
        if (clazz is JvmClassInfo) {
            return findOrAddClassInternal(clazz.jClass)
        }
