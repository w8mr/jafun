...
object IdentifierCache : SymbolMap {
    ...
    fun Type.JFClass.addField(name: String, type: OperandType<*>): Type.JFField {
    ...
    fun Type.JFClass.addMethod(
        name: String, arguments: List<Type.JFVariableSymbol>, returnType: OperandType<*>, field: Type.JFField? = null,
        static: Boolean = false, operator: Boolean = false, associativity: Associativity = Associativity.PREFIX,
        precedence: Int = 10
    ): Type.JFMethod {
    ...
    override fun find(type: TypeSymbol?, path: String): List<TypeSymbol> {
        ...
        return identifierMap[type]?.computeIfAbsent(path) { path ->
            when (type) {
                ...
                is OperandType.SInt32 -> listOf("jafun.lang.IntKt", "jafun.io.ConsoleKt", "jafun.test.TestKt").flatMap { findInClass(Class.forName(it), path)?.let { listOf(it) } ?: emptyList() }
    ...
    override fun contains(type: TypeSymbol?, path: String): kotlin.Boolean {
    ...
    private fun jfArray(returnName: String): OperandType.Array? = if (returnName.startsWith('[')) OperandType.Array(jvmType(returnName.substring(1))) else null //TODO: check implementation
